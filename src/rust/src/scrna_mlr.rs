...
impl<B: Backend + AutodiffBackend> TrainStep<SCBatch<B>, ClassificationOutput<B>> for Model<B> {
...
where
    B: Backend + AutodiffBackend + 'static,
    B::FloatElem: burn::tensor::backend::FloatElem,
    B::Device: Clone,
    SCBatcher<B>: Batcher<SCItem, SCBatch<B>>,
{
    type MyBackend = B;
...
// Specific functions for each backend
...
pub fn run_custom_wgpu(
    ...
    run_custom::<Wgpu<AutoGraphicsApi, f32>>(
        train,
        test,
        query,
        num_classes,
