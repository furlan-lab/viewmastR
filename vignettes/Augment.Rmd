...
cds<-readRDS(file.path(ROOT_DIR3, "210330_5p_cds.RDS"))
rownames(cds)<-SingleCellExperiment::rowData(cds)$gene_short_name
...
DimPlot_scCustom(seu, group.by = "viewmastR_smr")
```


## A confusion matrix showing prediction vs ground truth
```{r, dpi=300, fig.height=6, fig.width=10}
co<-as.character(pals::polychrome(length(levels(factor(c(seu$mCelltype, seu$viewmastR_smr))))))
names(co)<-levels(factor(c(seu$mCelltype, seu$viewmastR_smr)))
confusion_matrix(pred = factor(seu$viewmastR_smr), gt = factor(seu$mCelltype), cols = co)
Idents(seu)<-seu$viewmastR_smr
