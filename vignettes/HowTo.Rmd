...
Although we don’t know the cell type labels for the query dataset a priori, we can approximate the ground truth by using cluster-based cell type assignments. This approximation will help us evaluate the accuracy of viewmastR's predictions.

```{r}
# Harmonize labels
labels <- levels(factor(seur$SFClassification))
seu$ground_truth <- factor(seu$seurat_clusters)
levels(seu$ground_truth) <- c(
  labels[11], labels[18], labels[11], labels[17], labels[20],
  labels[21], labels[14], labels[12], labels[16], labels[18],
  labels[14], labels[19], labels[10], labels[11], labels[11]
)
...
We can visualize the query dataset with its ground truth labels to get an initial idea of the cell types we’re working with.
...
```{r}
seu <- viewmastR(seu, seur, ref_celldata_col = "SFClassification", selected_genes = vg, backend = "candle")
```
...
Finally, let’s include some session information for reproducibility.
