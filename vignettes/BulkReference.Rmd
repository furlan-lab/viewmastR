...
dat<-read.table("/Users/sfurlan/Library/CloudStorage/OneDrive-FredHutchinsonCancerCenter/computation/Analysis/ddata/MPAL/data/bulk/GSE74246_RNAseq_All_Counts.txt", header = T)
...
DimPlot_scCustom(seuHR, group.by = "viewmastR_smr", colors_use  = cols, label = T, repel = T, label.box = T, label.color = "white")
...
DimPlot_scCustom(seuP, group.by = "viewmastR_smr", colors_use = seuHR@misc$colors)
```
...
```{r}
Idents(seuP)<-seuP$viewmastR_smr
#levels(factor(as.character(seuP$viewmastR_smr)))
...
thresh<-100
seuP$pred_thresh<-seuP$viewmastR_smr
seuP$pred_thresh[seuP$pred_thresh %in% names(table(seuP$viewmastR_smr))[table(seuP$viewmastR_smr)<thresh]]<-"Other"
