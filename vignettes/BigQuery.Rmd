...
seu<-viewmastR_infer(seu[,1:100000], results[["model_dir"]], vg, labels = levels(factor(seur$SFClassification)), batch_size = 5024)
...
seu<-viewmastR_infer(seu[,1:100000], results[["model_dir"]], query_celldata_col = "viewmastR_inferred_parallel", vg, labels = levels(factor(seur$SFClassification)), chunks = 32, workers = 8, backend = "wgpu")

seu<-viewmastR_infer(seu[,1:100000], results[["model_dir"]], query_celldata_col = "viewmastR_inferred_parallel", vg, labels = levels(factor(seur$SFClassification)), chunks = 32, workers = 1, backend = "wgpu")


system.time({
  seu <- viewmastR_infer(seu[,1:100000], results[["model_dir"]], 
                         query_celldata_col = "test1", vg, 
                         labels = levels(factor(seur$SFClassification)), 
                         workers = 1, backend = "nd")
})
...
system.time({
  seu <- viewmastR_infer(seu[,1:100000], results[["model_dir"]], 
                         ...
                         labels = levels(factor(seur$SFClassification)), 
                         workers = 8, backend = "nd")
})

DimPlot(seu, group.by = "test1", cols = seur@misc$colors)
DimPlot(seu, group.by = "test2", cols = seur@misc$colors)
```
