...
scCustomize::FeaturePlot_scCustom(sc_data, features = "CDKN2A")
...
DimPlot(bmm)+NoLegend()

scCustomize::FeaturePlot_scCustom(bmm, features = "CDKN2A", raster = F)
scCustomize::FeaturePlot_scCustom(bmm, features = "CDKN2B", raster = F)
scCustomize::FeaturePlot_scCustom(bmm, features = "IRX1", raster = F)
scCustomize::FeaturePlot_scCustom(bmm, features = "MEN1", raster = F)
colnames(bmm@meta.data)
```
...
saveRDS(data, file.path("~/Desktop/converged.RDS"))
con <- as.vector(data)
...
Heatmap(data[!rownames(data) %in% "Intercept",], top_annotation = cola, show_column_names = F, col = viridis(100), bottom_annotation = HeatmapAnnotation(Intercept = anno_points(data[rownames(data) %in% "Intercept",])), name = "GD Result Conv")


data <- as.matrix(result_gd$proportions_with_intercept)
data <- data[,!(grepl("LSC", colnames(data)) | grepl("rHSC", colnames(data)) | grepl("Blast", colnames(data)))]
saveRDS(data, file.path("~/Desktop/no_converged.RDS"))
no_con <- as.vector(data)
cols <- pals::glasbey(13)
names(cols) <- unique(strsplit(colnames(data), "\\.") %>% sapply("[[", 2))
cola <- HeatmapAnnotation(bulk = strsplit(colnames(data), "\\.") %>% sapply("[[", 2), col = list(bulk=cols))
...
ggplot(data.frame(x = no_con, y = con), aes(x = x, y = y))+geom_point()
...
# Or separate them
cell_exposures <- result_em$exposures[1:21, ]
noise_exposures <- result_em$exposures[22, , drop=FALSE]
...
sort(avg_tumor_program, decreasing = TRUE)[1:20]
...
# Look for high expression + high correlation (e.g., cor > 0.5)
top_targets <- subset(target_candidates, Correlation > 0.3)
top_targets <- top_targets[order(top_targets$Expression, decreasing = TRUE), ]
